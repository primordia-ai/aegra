...
def get_auth_backend() -> AuthenticationBackend:
    ...
    Cached with lru_cache to ensure the auth file is loaded only once per process.
...
def get_auth_instance() -> Auth | None:
    ...
    Delegates to get_auth_backend() which is lru_cache'd, ensuring only one
    Auth instance is loaded per process.

    Returns:
        Auth instance or None if not configured/found
    """
    backend = get_auth_backend()
    return backend.auth_instance if isinstance(backend, LangGraphAuthBackend) else None
